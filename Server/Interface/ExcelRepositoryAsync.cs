...
namespace Server.Interface
{
    ...
    public interface IExcelRepositoryAsync
    {
        Task<string> ExportAsync<TData>(IEnumerable<TData> data
        , Dictionary<string, Func<TData, object>> mappers
        , string sheetName = "Sheet1");

        Task<Response<IEnumerable<TEntity>>> ImportAsync<TEntity>(Stream data
            , Dictionary<string, Func<DataRow, TEntity, object>> mappers
            , string sheetName = "Sheet1");
    }
    public class ExcelRepositoryAsync : IExcelRepositoryAsync
    {
        public async Task<string> ExportAsync<TData>(IEnumerable<TData> data, Dictionary<string, Func<TData, object>> mappers, string sheetName = "Sheet1")
        {
            ...
            p.Workbook.Properties.Author = "OneGuard";
        ...
        public async Task<Response<IEnumerable<TEntity>>> ImportAsync<TEntity>(Stream stream, Dictionary<string, Func<DataRow, TEntity, object>> mappers, string sheetName)
        {
            ...
            if (ws == null)
            {
                throw new ApiException(string.Format("Sheet with name {0} does not exist!", sheetName));
            }
            ...
            return new Response<IEnumerable<TEntity>>(result, "Import Success");
        }
    }
}
